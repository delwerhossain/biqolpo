...
export function SocialWall() {
  return (
    <section id="join" className="bg-cream py-14 md:py-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-5">
          {TILES.map((t) => (
            <a
              ...
              className="v3-card p-5 md:p-6 group"
            >
              <div className="flex items-center justify-between mb-4">
                <span className="w-10 h-10 rounded-full bg-primary-tint text-primary-dark flex items-center justify-center">
                  {t.icon}
                </span>
                <span className="v3-badge v3-badge-soft inline-flex items-center gap-1.5">
                  <span className="w-1.5 h-1.5 rounded-full bg-primary animate-pulse" />
                  Live
                </span>
              </div>

              <div className="text-[0.72rem] uppercase tracking-[0.15em] text-muted-2 mb-1">
                {t.platform}
              </div>
              <div className="font-display text-[1.35rem] md:text-[1.55rem] leading-tight text-text-deep mb-2">
                {t.handle}
              </div>
              <p className="text-[0.9rem] leading-[1.55] text-muted-2 mb-5">
                {t.blurb}
              </p>

              <div className="pt-4 border-t border-border-soft flex items-center justify-between">
                <span className="text-[0.82rem] font-semibold text-text-deep">
                  {t.cta}
                </span>
                <span className="text-primary-dark group-hover:translate-x-0.5 transition-transform">
                  →
                </span>
              </div>
            </a>
